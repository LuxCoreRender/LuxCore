...
Each environment map is loaded twice in memory. The `hdri` light uses a one-eighth-resolution copy for direct illumination and indirect diffuse and glossy rays; for example, a `16384 × 8192` source becomes `2048 × 1024`. The `hdri_background` light retains the original source pixel dimensions for sharp mirror-like reflections and the shadow-catcher ground's see-through areas, without adding diffuse or glossy illumination. When an external perspective FOV differs from 90°, the controller makes that light camera-invisible and alpha-composites a cached 90° HDRI projection behind the geometry instead, so changing the sender framing never changes the apparent HDRI size.
...
`lookat` (alias `cameraEyeTarget`) sets the view directly from `eye`, `target`, optional `up`, optional `fov` in degrees with an `axis` of `vertical` (default) or `horizontal`, and optional viewport `width` and `height` in pixels (16 to 8192; non-positive values are ignored). Its size fields set the visible viewport baseline and resize the render window; the active film follows the configured **Final Film Scale**. This differs from the standalone `resolution` command, which changes only the exact output film. The controller derives azimuth and elevation from the vectors, then halves the derived eye-to-target distance to match the sender's viewport framing. A supplied FOV controls only the LuxCore geometry projection: it is converted from the sender's stated vertical or horizontal axis to LuxCore's wider film axis, so the incoming view retains its intended framing. When that geometry FOV differs from 90°, the HDRI remains a fixed 90° wide-axis backplate, regenerated from the current camera orientation and composited behind transparent film pixels; lighting and reflections still use the HDRI itself. The `render_fov` response/status field exposes the sender-derived geometry value, while `background_fov` is the fixed 90° backdrop and `background_file` identifies its generated cache image. The controller ignores zero-length `up` vectors and non-positive `fov` values (as sent for orthographic viewports), and `reset` restores the scene's original up vector and field of view. A rolled `up` vector applies only until the next turntable-style interaction: the preset selector, orbit drags, and the `camera` command restore the world-Z up vector so canonical views stay level. Camera distance has no UI slider or range limit, so CAD-scale coordinates pass through exactly; no axis conversion is applied anywhere, meaning uploaded meshes and the camera share the sender's coordinate system.
...
- `define_material` stages a named LuxCore material from a `type` and a sparse `parameters` JSON object, then optionally assigns it to `object`. The command writes only the parameter keys the sender supplies, so LuxCore uses its built-in defaults for every omitted material property. `name` defaults to `<object>_mat` when omitted. Object and material names are normalized to LuxCore-safe identifiers, so `Body Panel 1` becomes `Body_Panel_1`.
...
For example, this defines only the four supplied `glossy2` properties and attaches the material to the uploaded object:
...
`parameters` may be omitted or contain any number of LuxCore material properties. Values may be numbers, booleans, strings, or flat scalar arrays. Use `base_material` to reference an already-defined base material for `glossycoating`; compound types with other reference names, such as `mix`, take their native properties (`material1`, `material2`, and `amount`) in `parameters`. The command is debounced with mesh uploads, then applies automatically; `apply` remains available when the sender needs an immediate restart.
