...
template<> inline bool Property::Get<bool>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<bool>(0);
}

template<> inline int Property::Get<int>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<int>(0);
}

template<> inline u_int Property::Get<u_int>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<u_int>(0);
}

template<> inline float Property::Get<float>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<float>(0);
}

template<> inline double Property::Get<double>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<double>(0);
}

template<> inline u_longlong Property::Get<u_longlong>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Get<u_longlong>(0);
}

template<> inline std::string Property::Get<std::string>() const {
	if (values.size() != 1)
		throw std::runtime_error("Wrong number of values in property: " + name);
...
template<> inline UV Property::Get<UV>() const {
	if (values.size() != 2)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return UV(Get<float>(0), Get<float>(1));
}

template<> inline Vector Property::Get<Vector>() const {
	if (values.size() != 3)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Vector(Get<float>(0), Get<float>(1), Get<float>(2));
}

template<> inline Normal Property::Get<Normal>() const {
	if (values.size() != 3)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Normal(Get<float>(0), Get<float>(1), Get<float>(2));
}

template<> inline Point Property::Get<Point>() const {
	if (values.size() != 3)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Point(Get<float>(0), Get<float>(1), Get<float>(2));
}

template<> inline Spectrum Property::Get<Spectrum>() const {
	if (values.size() != 3)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Spectrum(Get<float>(0), Get<float>(1), Get<float>(2));
}

template<> inline Matrix4x4 Property::Get<Matrix4x4>() const {
	if (values.size() != 16)
		throw std::runtime_error("Wrong number of values in property: " + name);
	return Matrix4x4(
...
template<> inline Property &Property::Add<UV>(const UV &v) {
	return Add(v.u).Add(v.v);
}

template<> inline Property &Property::Add<Vector>(const Vector &v) {
	return Add(v.x).Add(v.y).Add(v.z);
}

template<> inline Property &Property::Add<Normal>(const Normal &v) {
	return Add(v.x).Add(v.y).Add(v.z);
}

template<> inline Property &Property::Add<Point>(const Point &v) {
	return Add(v.x).Add(v.y).Add(v.z);
}

template<> inline Property &Property::Add<Spectrum>(const Spectrum &v) {
	return Add(v.c[0]).Add(v.c[1]).Add(v.c[2]);
}

template<> inline Property &Property::Add<Matrix4x4>(const Matrix4x4 &m) {
	for (u_int i = 0; i < 4; ++i) {
		for (u_int j = 0; j < 4; ++j) {
			Add(m.m[j][i]);
		}
...
template<> CPP_API void Property::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive &ar, const u_int version);
template<> CPP_API void Property::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive &ar, const u_int version) const;
...
template<> CPP_API void Properties::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive &ar, const u_int version);
template<> CPP_API void Properties::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive &ar, const u_int version) const;
