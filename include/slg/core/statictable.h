...
template <class T> class StaticTable {
public:
	StaticTable() { }
	~StaticTable() { }

	T Get(const std::string &name) const {
		const boost::unordered_map<std::string, T> &table = GetTable();

		typename boost::unordered_map<std::string, T>::const_iterator it = table.find(name);
		if (it == table.end())
			return NULL;
		else
			return it->second;
	...
	public:
		RegisterTableValue(const std::string &name, T val) {
			boost::unordered_map<std::string, T> &table = GetTable();
...
private:
	static boost::unordered_map<std::string, T> &GetTable() {
		...
		// instead of:
		//static boost::unordered_map<std::string, T> table;
...
#define STATICTABLE_DECLARE_DECLARATION(F) static StaticTable<F ## StaticTableType> STATICTABLE_NAME(F)
