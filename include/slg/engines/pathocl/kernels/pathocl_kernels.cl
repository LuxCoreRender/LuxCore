...
__kernel __attribute__((work_group_size_hint(64, 1, 1))) void InitFilm(
...
bool DirectLightSampling(
		...
		__global Ray *shadowRay, __global float *radiance, __global uint *ID
...
#endif
			{
				...
				if (BSDF_IsLightSource(bsdf)) {
					DirectHitFiniteLight((depth == 1), task->directLightState.lastBSDFEvent,
							...
							triLightDefs, &task->pathStateBase.throughput.r,
