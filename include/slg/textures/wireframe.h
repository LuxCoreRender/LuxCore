...
public:
	WireFrameTexture(const TextureMapping2D *mp, const float w,
			const Texture *borderTx, const Texture *insideTx) :
		mapping(mp), width(w), borderTex(borderTx), insideTex(insideTx) { }
	virtual ~WireFrameTexture() { delete mapping; }
