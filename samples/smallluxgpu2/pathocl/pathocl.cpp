...
PathOCLRenderEngine::PathOCLRenderEngine(Scene *scn, Film *flm, boost::mutex *filmMutex,
	...
	cerr << "Found "<< oclIntersectionDevices.size() << " OpenCL intersection devices for PathOCL render engine" << endl;

	if (oclIntersectionDevices.size() < 1)
		throw runtime_error("Unable to find an OpenCL intersection device for PathOCL render engine");
...
void PathOCLRenderEngine::Interrupt() {
	boost::unique_lock<boost::mutex> lock(engineMutex);
...
unsigned int PathOCLRenderEngine::GetThreadCount() const {
	boost::unique_lock<boost::mutex> lock(engineMutex);
