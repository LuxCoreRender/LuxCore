...
bool Scene_DefineBlenderCurveStrands(luxcore::detail::SceneImpl* scene,
	...
	//Points per strand
	extract<np::ndarray> getPointsperStrandArray(pointsPerStrand);
	if (!getPointsperStrandArray.check())
		throw runtime_error("PointsperStrand: not a numpy ndarray");
	...
	const np::ndarray& arrPointsperStrand = getPointsperStrandArray();
	const np::ndarray& arrPoints = getPointsArray();

	if (arrPointsperStrand.get_dtype() != np::dtype::get_builtin<int>())
		throw runtime_error("PointsperStrand: Wrong ndarray dtype (required: int32)");

	if (arrPointsperStrand.get_nd() != 1)
	...
	const int* const pointsperStrand = reinterpret_cast<const int*>(arrPointsperStrand.get_data());
	const float* const pointsStartPtr = reinterpret_cast<const float*>(arrPoints.get_data());
	const int pointStride = 3;
