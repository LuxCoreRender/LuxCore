...
void Film::Init() {
	...
	if (HasChannel(CONVERGENCE) && !convTest) {
		...
		convTest = new FilmConvTest(this, haltNoiseThreshold, haltNoiseThresholdWarmUp, haltNoiseThresholdTestStep, haltNoiseThresholdUseFilter, haltNoiseThresholdIndex);
	}

	// NOISE is generated by the noise estimation test
	if (HasChannel(NOISE) && !noiseEstimation)
		noiseEstimation = new FilmNoiseEstimation(this, noiseEstimationWarmUp, noiseEstimationTestStep, noiseEstimationFilterScale, noiseEstimationIndex);
...
void Film::RunTests() {
	...
	if (noiseEstimationRequired) {
		ExecuteImagePipeline(noiseEstimationIndex);
