...
void FilmNoiseEstimation::Test() {
	...
	if (film->GetTotalSampleCount() / pixelsCount <= warmup) return;

	// Do not run the test if we don't have at least adaptivesampling.step new samples per pixel
	const double newSamplesCount = film->GetTotalSampleCount();
	if (newSamplesCount  - lastSamplesCount <= pixelsCount * static_cast<double>(testStep)) return;
	...
	if (firstTest) {
		...
		SLG_LOG("Noise estimation for adaptive sampling: first pass");

		return;
	} else {
		SLG_LOG("Noise estimation for adaptive sampling: test");
