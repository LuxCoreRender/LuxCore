...
Material *Scene::CreateMaterial(const u_int defaultMatID, const string &matName, const Properties &props) {
	...
	mat->SetEmittedPower(Max(0.f, props.Get(Property(propName + ".emission.power")(100.f)).Get<float>()));
	mat->SetEmittedEfficency(Max(0.f, props.Get(Property(propName + ".emission.efficency")(17.f)).Get<float>()));
